...
void timeInterrupt() {
  ...
  minutes = (timer / 60000) % 60;
  hours = (timer / 3600000) % 24;
  ...
  if(stopwatchCounting) {
    ...
    if(stopwatchTimer > 86400000-1) { // Reset stopwatch when "full"
      stopwatchTimer = stopwatchTimer - 86400000;
    }

    if(!stopwatchLapping) { // Check for lapping
      stopwatchCentiseconds = (stopwatchTimer / 10 ) % 100;
      stopwatchSeconds = (stopwatchTimer / 1000) % 60;
      stopwatchMinutes = (stopwatchTimer / 60000) % 60;
      stopwatchHours = (stopwatchTimer / 3600000) % 60;
    }
  }
  ...
  switch (programMode) {
    ...
    case 3: // Set Alarm.
      ...
      if(setAlarmMode == 1 && input == buttonB) {
        alarmTime += 3600000 * boost;
        if(alarmTime > 86400000-1) alarmTime -= 86400000;
      }

      if(setAlarmMode == 2 && input == buttonB) {
        alarmTime += 60000 * boost;
        if((alarmMinutes / 60000) > 59) {
          alarmTime -= 3660000;
        }
      }

      alarmHours = (alarmTime / 3600000) % 24;
      alarmMinutes = (alarmTime / 60000) % 60;
...
void updateDisplay() {
  ...
  switch(programMode) {
    ...
    setCharDisplay(1, stopwatchMinutes/10);
    setCharDisplay(2, stopwatchMinutes%10);
    setCharDisplay(3, stopwatchSeconds/10);
    setCharDisplay(4, stopwatchSeconds%10);
    setCharDisplay(5, stopwatchCentiseconds/10);
    setCharDisplay(6, stopwatchCentiseconds%10);
